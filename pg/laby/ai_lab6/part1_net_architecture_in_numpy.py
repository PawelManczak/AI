...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            """
            # TODO (0.5 point)
            logits = np.matmul(x_data, self.W) + self.b
...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            m_data = self.hidden_layer.forward(x_data)
            return self.output_layer.forward(m_data)
    ...
    model.hidden_layer.W[:, 0] = -1, 10# wagi neuronu h1
    model.hidden_layer.W[:, 1] = 11, -15# wagi neuronu h2
    model.hidden_layer.b[:] = -1, 10  # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = 80, 23   # wagi neuronu wyjściowego
    model.output_layer.b[:] = -92# bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 188756         # Twój numer indeksu, np. 102247
